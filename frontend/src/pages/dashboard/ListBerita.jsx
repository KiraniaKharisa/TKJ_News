...
const ListBerita = () => {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold mb-2">List Berita</h1>
      <p>Menampilkan tentang berita di platform ini</p>
      <div className="bg-[rgb(32_31_31)] px-5 py-3 my-5 rounded-lg shadow shadow-blue-500/25">
        <div className="flex justify-between">
            <h1 className="text-lg font-bold mt-2">List Berita Platform</h1>
              <div className="flex items-center gap-x-2 border-1 border-gray-200 px-2 py-1 rounded">
                  <Search size={18}/>
                  <input type="text" className="outline-none placeholder:text-[13px]" autoFocus placeholder="Cari Berita Sesuai Judul..."  onChange={(e) => setSearch(e.target.value)}/>
              </div>
        </div>
                  ...
                  <tr className="table-row" key={i}>
                    ...
                    <td className="table-cell-me flex items-center gap-x-2">
                      ...
                      <div>
                        ...
                        <span className="text-sm text-gray-400">{item.kategori.map(k => k.kategori).join(', ')} - {formatDateCreatedAt(item.created_at, {nameMonth: true})} - Views : {formatNumberLikes(item.views)}</span>
                      </div>
                    </td>
                    <td className="table-cell-me text-center">{item.user.name}</td>
